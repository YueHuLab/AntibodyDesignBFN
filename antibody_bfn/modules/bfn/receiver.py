...
from antibody_bfn.modules.encoders.ga import GAEncoder
from antibody_bfn.modules.bfn.orientation import svd_project_so3
...
class AntibodyBFN_Receiver(nn.Module):
    def __init__(self, res_feat_dim, pair_feat_dim, num_layers, encoder_opt={}, num_classes=20, seq_only=False):
        ...
        self.angle_embed = nn.Linear(4 * 2, res_feat_dim) # Concatenate sin/cos of angles?
        # Or if we use GMM parameters: K * (1 + 1 + 1)? 
        # Prompt: "Concatenate(sum(pi * mu), pi)" or "K gaussian means weighted".
        # Let's stick to a simple embedding of the expected angle for now or the full parameters if dimensions allow.
        # Given GAEncoder structure, we feed 'res_feat' which combines sequence and other attributes.
        ...
        self.res_mixer = nn.Sequential(
            nn.Linear(res_feat_dim * 3, res_feat_dim), # seq + angle + time
            nn.ReLU(),
            nn.Linear(res_feat_dim, res_feat_dim)
        )
    ...
    def forward(self, theta_seq, theta_pos, theta_ori, theta_ang, t, pair_feat, mask_res, 
        ...
        # Mix features
        res_feat = self.res_mixer(torch.cat([emb_seq, emb_ang, emb_t], dim=-1))
        ...
        # Seq-only mode: skip structure predictions to save compute
        if self.seq_only:
            pred_pos = pos  # Just return input
            pred_ori_6d = torch.zeros(N, L, 6, device=device)
            pred_ang_sc = torch.zeros(N, L, 8, device=device)

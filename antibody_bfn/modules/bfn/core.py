...
from antibody_bfn.modules.bfn.sequence import CategoricalBFN
from antibody_bfn.modules.bfn.position import PositionBFN
from antibody_bfn.modules.bfn.orientation import OrientationBFN
from antibody_bfn.modules.bfn.sidechain import SidechainBFN
from antibody_bfn.modules.bfn.receiver import AntibodyBFN_Receiver
from antibody_bfn.modules.common.geometry import repr_6d_to_rotation_matrix, compute_fape
from antibody_bfn.modules.common.ot_align import LieOTAlign

class AntibodyBFN_Core(nn.Module):
    def __init__(self, res_feat_dim, pair_feat_dim, num_steps, eps_net_opt={}, position_mean=[0.0, 0.0, 0.0], position_scale=[10.0], loss_weight={}, ot_opt={}):
        super().__init__()
        ...
        self.flow_seq = CategoricalBFN(num_classes=self.num_classes, num_steps=num_steps)
        self.flow_pos = PositionBFN()
        self.flow_ori = OrientationBFN()
        self.flow_ang = SidechainBFN()
        
        # Detect seq-only mode (no structure losses)
        seq_only = not any(k in loss_weight and loss_weight.get(k, 0) > 0 for k in ['dist', 'fape', 'ang', 'pos', 'rot'])
    ...
    def forward(self, batch):
        ...
        # Construct Rotation
        from antibody_bfn.modules.common.geometry import construct_3d_basis
        ...
        inp_ang = self.flow_ang.get_angle(theta_ang_full)
    ...
    def sample(self, batch, sample_opt={}):
        ...
        for i in range(1, steps + 1):
            ...
            # For seq-only mode: always use true structure
            if seq_only:
                inp_pos = x_pos
                inp_ori = x_ori
                inp_ang = x_ang
        ...
        from antibody_bfn.modules.common.so3 import rotation_to_so3vec
